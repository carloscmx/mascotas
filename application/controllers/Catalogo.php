...
class Catalogo extends CI_Controller
{
	...
	public function guardarAnimal()
	{
		if ($this->input->is_ajax_request()) {
			...
			$this->form_validation->set_rules('txtNombrepet', 'Nombrepet', 'required');
			$this->form_validation->set_rules('cbEspecie', 'Especie', 'required');
			$this->form_validation->set_rules('cbRaza', 'Raza', 'required');
			$this->form_validation->set_rules('ddtFechanan', 'Fechanac', 'required');
			$this->form_validation->set_rules('txtColor', 'Color', 'required');
			$this->form_validation->set_rules('cboSexo', 'Sexo', 'required');
			...
			if ($this->form_validation->run()) {
				...
				$this->output->set_status_header(400)->set_content_type('application/json')->set_output(json_encode([
					'status' => false,
					'message' => [
						'Nombrepet' => form_error('txtNombrepet'),
						'Especie' => form_error('cbEspecie'),
						'Raza' => form_error('cbRaza'),
						'Fechanac' => form_error('ddtFechanan'),
						'Color' => form_error('txtColor'),
						'Sexo' => form_error('cboSexo')
					]
				]));
			}
		} else {
